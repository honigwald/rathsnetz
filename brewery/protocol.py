...
def processing_pdf(cid):
    ...
    logo = Image(
        os.path.join(BASE_DIR, "static/img/logo-white-long.png"), width=384, height=80
    )
    ...
    p1text.append(
        Paragraph(
            "<u>Braumeister</u>: " + str(c.brewmaster), sample_style_sheet["BodyText"]
        )
    )
    ...
    p1text.append(
        Paragraph("<u>Ausstoß</u>: " + str(c.output), sample_style_sheet["BodyText"])
    )
    p1text.append(
        Paragraph(
            "<u>Restextrakt</u>: " + str(c.restextract), sample_style_sheet["BodyText"]
        )
    )
    p1text.append(
        Paragraph(
            "<u>Stammwürze</u>: " + str(c.reached_wort), sample_style_sheet["BodyText"]
        )
    )
